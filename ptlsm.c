...
/*  
struct cert_pt *new_cert_pt()
{
	struct cert_pt *pt_cert;

	pt_cert = kzalloc(sizeof(struct cert_pt), GFP_NOFS);
	if (pt_cert == NULL)
		return NULL;
...
{
	int err;
	err = vfs_getxattr(dentry, "security.tdm", degr, len);
	if(err<0){
		printk("getxattr failed!\n");
		return err;
	}
	return 0;
}

static int umh_test(char *path)
{
	struct subprocess_info *sub_info;
	char *argv[] = { "/usr/bin/datdm", path, NULL};
	static char *envp[] = {
		"HOME=/",
		"TERM=linux",
...
static struct cert_pt *pt_fetch(struct inode *ip, struct dentry *dp)
{
	struct cert_pt *rc;
	struct inode_pt *ipp;
...
{ 
	...
	ipp = kzalloc(sizeof(struct inode_pt), GFP_NOFS);
	if (ipp == NULL)
		return NULL;
...
{
	struct task_pt *tpp;
	tpp = kzalloc(sizeof(struct task_pt), gfp);
...
static int ptlsm_bprm_set_creds(struct linux_binprm *bprm)
{
	...
	if (err < 0){
		...
		printk(KERN_INFO "TDM KERN: degree: %d, status: %d\n",
			degr->deg, degr->sts);
		if(!degr->sts){
			printk(KERN_INFO "TDM KERN: permission deny\n");
			return -EACCES;
		}
	}
...
static int ptlsm_file_permission(struct file *file, int mask)
{
	...
	if(!tdm_flag) return -ENOMEM;
	size = sizeof(int);
	err = vfs_getxattr(file->f_path.dentry, "security.tdmflag", tdm_flag, size);
	if (err < 0){
		goto OUT;	
	} else {
		fullpath = kmalloc(PATH_MAX, GFP_KERNEL);
...
{
	/*
	struct task_pt *old_tpp = old->security;
	struct task_pt *new_tpp;

	new_tpp = new_task_pt(old_tpp->pt_cert, gfp);
	new->security = new_tpp;
	
	printk("cred_prepare: uid, %d; pt_cert, %s\n", new->uid, new_tpp->pt_cert->cert_name);
	*/
	return 0;
}
...
static void ptlsm_cred_transfer(struct cred *new, const struct cred *old)
{
	/*  
	struct task_pt *old_tpp = old->security;
	struct task_pt *new_tpp = new->security;

	new_tpp->pt_cert = old_tpp->pt_cert;

  printk("cred_transfer\n");
	*/
}
...
static __init int ptlsm_init(void)
{
	...
	if (!security_module_enable(&ptlsm_ops))
	{
			printk("ptlsm enable failed!\n");
			return 0;
	}
	
	/*  
	cpp = kzalloc(sizeof(struct cert_pt), GFP_KERNEL);

	printk("init process setting attribute...\n");

	if (cpp == NULL)
	{
		printk("cpp kmalloc failed!\n");
		return -ENOMEM;
	}
	cpp->cert_no = 0;
	cpp->cert_name = kmalloc(sizeof(char[20]), GFP_KERNEL);
	cpp->cert_name = "INIT PROCESS";
	tpp = new_task_pt(cpp, GFP_KERNEL);
	
	if (tpp == NULL)
	{
		printk("tpp init failed!\n");
		return -ENOMEM;
	}

	printk(KERN_INFO "ptsec: Initializing.\n");
